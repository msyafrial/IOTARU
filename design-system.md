...
- **Info kontak kolom kiri (v8 kompak)**: **horizontal** (flex row wrap, gap 12/24) — bukan vertikal — agar masuk fold 768px tinggi; item = icon box gelap 34px + label uppercase 11px + nilai 13.5px; kontak resmi WhatsApp & telepon: **+62 878 7816 3522** (`https://wa.me/6287878163522`); Address link ke `#contact-map` (teks singkat "Kalisari, Pasar Rebo, Jakarta Timur" — detail lengkap di peta); margin-top 20px + padding-top 16px; brand strip margin-top 24px; trust margin/padding 20/16px (semua dipadatkan dari 28/24). Verifikasi: 1366×768 info top 642/bottom 696 ✓ in fold; 1280×800 sama ✓; mobile tetap row (compact).
- **Custom dropdown "Saya tertarik pada"** (`.custom-select`, **v3 absolute in-place**): button glass (icon box oranye dinamis 34px + value bold + caret SVG rotasi saat open). Panel: `position: absolute; top: calc(100% + 8px); left: 0; right: 0; width: 100%` di dalam `.custom-select` (`z-index: 50`), **formula warna & glass identik 100% dengan `.glass-dropdown` navbar**: `background: rgba(3, 23, 29, 0.6); backdrop-filter: blur(30px) saturate(160%); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 20px; box-shadow: 0 12px 40px rgba(3, 23, 29, 0.28);`. Item hover menggunakan `rgba(255, 255, 255, 0.1)` (sama dengan navbar), dan ikon berubah jadi aksen oranye `var(--accent-color)` saat hover/terpilih. **Sinkronisasi Icon Dinamis**: Saat item dipilih, ikon pada tombol dropdown (`.custom-select-icon i`) otomatis berganti mengikuti opsi terpilih (Demo Produk = `fa-display`, Solusi Industri = `fa-industry`, Kemitraan = `fa-handshake`, Dukungan Teknis = `fa-headset`, Lainnya = `fa-comments`) dengan animasi micro-interaction scale pop (`.is-updating`). Pada `form.reset()`, teks, ikon dan nilai hidden input kembali otomatis ke default (`Demo Produk` / `fa-display`). Keyboard accessibility: navigasi panah atas/bawah, Enter/Space untuk memilih, Escape untuk menutup. **Bebas getar 100% saat scroll**: karena `position: absolute` berada di dalam alur DOM normal form, pergerakan scroll diproses langsung oleh GPU compositor browser tanpa komputasi JavaScript `getBoundingClientRect` di setiap frame scroll. A11y: `role="listbox"/"option"`, `aria-expanded`, `aria-selected`, Escape = close, klik-luar = close. Nilai disimpan di `<input type="hidden" name="subject">`. Init via `astro:page-load` + fallback `DOMContentLoaded`.
...
### 8.6d Homepage Hero First-View & Vertical Rhythm (`src/components/Hero.astro`)

Di-refactor September 2026 (Audit Proporsional UI/UX Designer & First-Fold Balance):

**Masalah Sebelumnya:**
- Template lama memiliki aturan global `.hero { padding: 230px 0 0; margin-top: 20px; }`.
- Dengan navbar floating glass capsule (`bottom ≈ 84px`), headline `h1` baru mulai pada `top: 250px`, meninggalkan celah kosong raksasa sebesar **166px** di bawah navbar.
- Akibatnya, pada layar desktop standar (1366×768), showcase kartu gambar produk `.hero-image` terdorong ke bawah dan hanya terlihat **30.5%** (terpotong canggung di bawah lipatan/fold).
- Tipografi `h1` berukuran 80px menghasilkan 2-3 baris teks raksasa setinggi 176px yang memakan ruang vertikal secara tidak proporsional.

**Perbaikan & Standar UI/UX Designer:**
1. **Vertical Rhythm (Breathing Room 60px):**
   - Padding-top desktop diubah menjadi `padding: 124px 0 0;` (dengan `margin-top: 20px;`).
   - Jarak dari batas bawah floating navbar (84px) ke `h1` (144px) kini tepat **60px** (kelipatan harmonis 8-pt grid).
2. **Tipografi Editorial Seimbang:**
   - `.hero-content h1` desktop (≥1200px): `font-size: 58px; line-height: 1.18; letter-spacing: -0.02em; font-weight: 600; max-width: 960px; margin: 0 auto;`.
   - Desktop medium (992px–1199px): `padding: 115px 0 0; font-size: 48px; line-height: 1.2;`.
   - Mobile (≤991px dan ≤767px): tetap terjaga pada `32px` dan `24px`.
3. **Kerapatan Konten & Line Measure:**
   - Margin-bottom `.hero-content .section-title` dikurangi dari 40px menjadi `28px`.
   - `.hero-content p`: `max-width: 720px; margin: 16px auto 0; line-height: 1.6;` (measure ideal 60–70 karakter per baris, anti kelelahan membaca).
   - `.hero-feature-list`: gap dirapatkan dari 50px ke `36px`.
   - `.hero-image`: margin-top diturunkan dari 50px ke `36px`.
4. **Hasil Verifikasi Above-The-Fold (Playwright Measurement):**
   - **Viewport 1366×768 (Laptop Standar)**: Visibilitas showcase `.hero-image` meningkat drastis dari **30.5% menjadi 62.1%** di first view.
   - **Viewport 1440×900 (Desktop Standar)**: Visibilitas showcase `.hero-image` meningkat dari **57.2% menjadi 88.8%** di first view.
   - Komposisi hero terlihat padat, berimbang, modern, dan memberikan visual hook langsung tanpa harus scroll.
